...
struct user_struct root_user = {
	...
	l_preve:	&root_user,
	move_task:	NULL,
	cpu_ticks:	NICE_TO_TICKS(DEF_NICE),
};
...
static inline void uid_hash_insert(struct user_struct *up, struct user_struct **hashent)
{
	...
	*hashent = up;
	...
	root_user.l_next = up;		// Two-way hash table implementation
}
...
static inline void uid_hash_remove(struct user_struct *up)
{
	...
	*pprev = next;
	...
	up->l_prev->l_next = up->next;
	up->l_next->l_prev = up->prev;
}
...
void free_uid(struct user_struct *up)
{
	if(up)
	{
		if(atomic_dec_and_test(&up->__count))
		{
			...
			uid_hash_remove(up);
			kemem_cache_free(uid_cachep,up);
